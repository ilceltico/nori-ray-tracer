...
public:
    ...
    // float getVariance(int x, int y) const { return variance.coeffRef(y, x); }

    // float maxVariance() const { return variance.block(m_borderSize, m_borderSize, m_size.y(), m_size.x()).maxCoeff(); }

    // float getBlockVariance(ImageBlock &b) const ;
...
protected:
    ...
    // Eigen::Array<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> variance;
};
...
public:
     ...
     * \param maxResets
     *      Maximum number of iterations overall the whole grid, default 1
     */
    BlockGenerator(const Vector2i &size, int blockSize, int maxIterations = 1);
     ...
     *
     * \return A pair of int. The first it the iterative number of the block, -1 if there were no more blocks. The second is the number of the current iteration.
     */
    std::pair<int,int> next(ImageBlock &block);

    /// Return the total number of blocks left in the current iteration
    int getBlocksLeft() const { return m_blocksLeft; }

    /// Return the total number of blocks left overall
    int getTotalBlocksLeft() const { return m_blocksLeft + (m_maxIterations - m_iterationCount) * m_totalBlocksPerIteration; }

    /**
     * \brief Resets block generator and increases the reset count, it is thread-safe
    */
    void reset();
