 ...
 *      Cosine of the angle between the normal and the incident ray
 * \param eta
 *      Refractive index of the metal divided by the index of the surrounding medium
 * \param k
 *      Imaginart part of the refractive index of the metal, divided by the index of the surrounding medium
 */
extern Color3f fresnelConductor(float cosThetaI, const Color3f &eta, const Color3f &k);
// float fresnelConductor(float cosThetaI, float eta, float k);

/**
 * \brief Computes a suitable Color3f representation for Eta and K parameters for metals, starting from a sampled representation
*/
extern Color3f fromSampledConductor(const float *lambda, const float *v, int n);
...
/**
 * \brief Reads a sampled representation from txt file line by line.
 * 
 * \param filename
 *      Resolved filename
 * \param lambdas
 *      Empty vector to be filled with wavelength values
 * \param values
 *      Empty vector to be filled with values
*/
extern void readSampledConductor(std::string filename, std::vector<float> &lambdas, std::vector<float> &values);
...
/**
 * Prints a progress bar with the specified parameters
*/
extern void printProgress(double current, double max, Timer &timer);
