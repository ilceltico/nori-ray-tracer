...
void Scene::activate() {
    ...
    for (uint32_t i=0; i<this->getMeshes().size(); i++) {
        if (this->getMeshes()[i]->isEmitter()){
            m_emitters.push_back(this->getMeshes()[i]->getEmitter());
        }
    }

    for (uint32_t i=0; i<this->m_emitters.size(); i++) {
        m_emitters[i]->preprocess(*this);

        if (m_emitters[i]->isEnvironmentMap()) {
            if (!m_environmentMap)
                m_environmentMap = m_emitters[i];
            else
                throw NoriException("Scene: multiple environment maps defined!");
        }
    }
