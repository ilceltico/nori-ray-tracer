...
ImageBlock::ImageBlock(const Vector2i &size, const ReconstructionFilter *filter) 
    ...
    // variance.resize(size.y(), size.x());
    // variance.setConstant(100.0f);
}
...
void ImageBlock::put(const Point2f &_pos, const Color3f &value) {
    ...
    // Color4f beforeColor = coeffRef(pos.y(), pos.x());
    // float beforeValue = beforeColor.x() * beforeColor.x() + beforeColor.y() * beforeColor.y() + beforeColor.z() * beforeColor.z();
    ...
    // Color4f afterColor = coeffRef(pos.y(), pos.x());
    // float afterValue = afterColor.x() * afterColor.x() + afterColor.y() * afterColor.y() + afterColor.z() * afterColor.z();

    // variance.coeffRef(pos.y(),pos.x()) = (afterValue - beforeValue) / 2.0f;
}
...
void ImageBlock::put(ImageBlock &b) {
    ...
    // Vector2i offsetVariance = b.getOffset();
    // for (int x=offsetVariance.x(); x<offsetVariance.x() + b.getSize().x(); x++) {
    //     for (int y=offsetVariance.y(); y<offsetVariance.y() + b.getSize().y(); y++){
    //         if (b.coeffRef(y - offsetVariance.y() + b.getBorderSize(), x - offsetVariance.x() + b.getBorderSize()).w() == 0)
    //             continue;

    //         Color4f beforeColor = coeffRef(y, x);
    //         float beforeValue = std::sqrt(beforeColor.x() * beforeColor.x() + beforeColor.y() * beforeColor.y() + beforeColor.z() * beforeColor.z());

    //         Color4f afterColor = coeffRef(y, x) + b.coeffRef(y - offsetVariance.y() + b.getBorderSize(), x - offsetVariance.x() + b.getBorderSize());
    //         float afterValue = std::sqrt(afterColor.x() * afterColor.x() + afterColor.y() * afterColor.y() + afterColor.z() * afterColor.z());

    //         float newVariance = std::abs(afterValue - beforeValue) / 2.0f;
    //         // if (newVariance < variance.coeffRef(y,x))
    //         variance.coeffRef(y, x) = newVariance / b.coeffRef(y - offsetVariance.y() + b.getBorderSize(), x - offsetVariance.x() + b.getBorderSize()).w();
    //     }
    // }
...
// float ImageBlock::getBlockVariance(ImageBlock &b) const {
//     return variance.block(b.getOffset().y(), b.getOffset().x(), b.getSize().y(), b.getSize().x()).mean();
// }
...
BlockGenerator::BlockGenerator(const Vector2i &size, int blockSize, int maxIterations)
    ...
    m_totalBlocksPerIteration = m_blocksLeft;
    m_maxIterations = maxIterations;

    m_timer.reset();
}

std::pair<int,int> BlockGenerator::next(ImageBlock &block) {
    tbb::mutex::scoped_lock lock(m_mutex);

    if (m_blocksLeft == 0) {
        if (m_maxIterations > m_iterationCount) {
            m_numBlocks = Vector2i(
                (int) std::ceil(m_size.x() / (float) m_blockSize),
                (int) std::ceil(m_size.y() / (float) m_blockSize));
            m_blocksLeft = m_numBlocks.x() * m_numBlocks.y();
            m_direction = ERight;
            m_block = Point2i(m_numBlocks / 2);
            m_stepsLeft = 1;
            ...
            std::cout << "Sample " << m_iterationCount << "/" << m_maxIterations << "\n";
        }
        else
            return std::pair<int,int> (-1, m_iterationCount);
    }
    ...
    if (--m_blocksLeft == 0)
        return std::pair<int,int> (m_totalBlocksPerIteration - m_blocksLeft - 1, m_iterationCount);
    ...
    printProgress(m_totalBlocksPerIteration * m_maxIterations - getTotalBlocksLeft() + 1, m_totalBlocksPerIteration * m_maxIterations, m_timer);
