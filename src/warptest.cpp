...
static const std::string kWarpTypeNames[WarpTypeCount] = {
    ...
    "cosine_hemisphere", "beckmann", "hierarchical_sample", "microfacet_brdf", "microfacet_dielectric", "hg"
};
...
struct WarpTest {
    ...
    WarpTest(WarpType warpType_, float parameterValue_, std::string exrFilename, BSDF *bsdf_ = nullptr,
             BSDFQueryRecord bRec_ = BSDFQueryRecord(nori::Vector3f()),
             PhaseFunction *phaseFunction_ = nullptr,
             int xres_ = kDefaultXres, int yres_ = kDefaultYres)
        : warpType(warpType_), parameterValue(parameterValue_), bsdf(bsdf_),
          bRec(bRec_), phaseFunction(phaseFunction_), xres(xres_), yres(yres_) {

        if (!bRec.sampler) {
            bRec.sampler = static_cast<Sampler*>(NoriObjectFactory::createInstance("independent", PropertyList()));
        }

        /* Initialize the Hierarchical Sample Warping Mipmap */
        if (warpType == HierarchicalSampleWarping) {
            mipmap = Mipmap(exrFilename);
            xres = 1024 ;
            yres = 1024 ;
        }

        if (warpType != Square && warpType != Disk && warpType != Tent && warpType != HierarchicalSampleWarping)
            xres *= 2;
    ...
    std::pair<bool, std::string> run() {
        ...
        for (int i=0; i<sampleCount; ++i) {
            ...
            if (warpType == Square || warpType == HierarchicalSampleWarping) {
                x = sample.x();
                y = sample.y();
        ...
        auto integrand = [&](double y, double x) -> double {
            if (warpType == Square) {
            ...
            } else if (warpType == HierarchicalSampleWarping) {
                return Warp::squareToHierarchicalSampleWarpingPdf(Point2f(x, y), mipmap);
                ...
                else if (warpType == MicrofacetDielectric) {
                    BSDFQueryRecord br(bRec);
                    br.wo = v;
                    br.measure = nori::ESolidAngle;
                    return bsdf->pdf(br);
                } else if (warpType == HenyeyGreenstein) {
                    return phaseFunction->pdf(bRec.wi, v);
                } else {
    ...
    std::pair<Point3f, float> warpPoint(const Point2f &sample) {
        ...
        switch (warpType) {
            ...
            case HierarchicalSampleWarping:
                result << Warp::squareToHierarchicalSampleWarping(sample, mipmap), 0; break;
            case MicrofacetBRDF: {
                BSDFQueryRecord br(bRec);
            ...
            case HenyeyGreenstein: {
                nori::Vector3f wo;
                float value = phaseFunction->sample(bRec.wi, &wo, sample);
                return std::make_pair(
                    wo,
                    value
                );
    ...
    create_microfacet_bsdf(float alpha, float kd, float bsdfAngle) {
        ...
        // bRec.sampler = static_cast<Sampler*>(
        //     NoriObjectFactory::createInstance("independent", PropertyList()));
        return { brdf, bRec };
    }

    static std::pair<BSDF *, BSDFQueryRecord>
    create_microfacet_dielectric(float alpha, float bsdfAngle) {
        PropertyList list;
        list.setFloat("alpha", alpha);
        auto * brdf = (BSDF *) NoriObjectFactory::createInstance("microfacet_dielectric", list);
    ...
    static std::pair<PhaseFunction *, BSDFQueryRecord>
    create_henyey_greenstein(float g, float angle) {
        PropertyList list;
        list.setFloat("g", g);
        auto * phaseF = (PhaseFunction *) NoriObjectFactory::createInstance("hg", list);

        nori::Vector3f wi(std::sin(angle), 0.f,
                    std::max(std::cos(angle), 1e-4f));
        wi = wi.normalized();
        BSDFQueryRecord bRec(wi);
        return { phaseF, bRec };
    }
...
public:
    ...
    static float mapParameter(WarpType warpType, float parameterValue) {
        if (warpType == Beckmann || warpType == MicrofacetBRDF || warpType == MicrofacetDielectric)
    ...
    void refresh() {
        ...
        if (warpType == MicrofacetDielectric) {
            BSDF *ptr;
            float bsdfAngle = M_PI * (m_angleSlider->value() - 0.5f);
            std::tie(ptr, m_bRec) = WarpTest::create_microfacet_dielectric(
        ...
        if (warpType == HenyeyGreenstein) {
            PhaseFunction *ptr;
            float angle = M_PI * (m_angleSlider->value() - 0.5f);
            std::tie(ptr, m_bRec) = WarpTest::create_henyey_greenstein(
                parameterValue, angle);
            m_phaseFunction.reset(ptr);
        }

        /* Generate the point positions */
        nori::MatrixXf positions, values;
        try {
            WarpTest tester(warpType, parameterValue, exrFilename, m_brdf.get(), m_bRec, m_phaseFunction.get());
        ...
        if (!m_brdfValueCheckBox->checked() || (warpType != MicrofacetBRDF && warpType != MicrofacetDielectric && warpType != HenyeyGreenstein))
            value_scale = 0.f;
        ...
        m_parameterSlider->set_enabled(warpType == Beckmann || warpType == MicrofacetBRDF || warpType == MicrofacetDielectric || warpType == HenyeyGreenstein);
        m_parameterBox->set_enabled(warpType == Beckmann || warpType == MicrofacetBRDF || warpType == MicrofacetDielectric || warpType == HenyeyGreenstein);
        m_parameter2Slider->set_enabled(warpType == MicrofacetBRDF);
        m_parameter2Box->set_enabled(warpType == MicrofacetBRDF);
        m_angleBox->set_enabled(warpType == MicrofacetBRDF || warpType == MicrofacetDielectric || warpType == HenyeyGreenstein);
        m_angleSlider->set_enabled(warpType == MicrofacetBRDF || warpType == MicrofacetDielectric || warpType == HenyeyGreenstein);
        m_brdfValueCheckBox->set_enabled(warpType == MicrofacetBRDF || warpType == MicrofacetDielectric);
    ...
    void draw_contents() {
        ...
        if (m_drawHistogram) {
            ...
            const int histHeight = (warpType == Square || warpType == Disk || warpType == Tent || warpType == HierarchicalSampleWarping) ? histWidth : histWidth / 2;
            ...
            if (m_warpTypeBox->selected_index() == MicrofacetDielectric) {
                m_arrowShader->set_uniform("mvp", mvp);
                m_arrowShader->begin();
                m_arrowShader->draw_array(nanogui::Shader::PrimitiveType::Line, 0, 106);
                m_arrowShader->end();
            }
            if (m_warpTypeBox->selected_index() == HenyeyGreenstein) {
                m_arrowShader->set_uniform("mvp", mvp);
                m_arrowShader->begin();
                m_arrowShader->draw_array(nanogui::Shader::PrimitiveType::Line, 0, 106);
                m_arrowShader->end();
    ...
    void initializeGUI() {
        ...
        m_warpTypeBox = new ComboBox(m_window, { "Square", "Tent", "Disk", "Sphere", "Hemisphere (unif.)",
                "Hemisphere (cos)", "Beckmann distr.", "Hierarchical Sample Warping", "Microfacet BRDF", "Microfacet Dielectric", "Henyey Greenstein" });
        m_warpTypeBox->set_callback([&](int) { 
                WarpType warpType = (WarpType) m_warpTypeBox->selected_index();
                if (warpType == HierarchicalSampleWarping)
                    exrFilename = nanogui::file_dialog({{std::string("exr"), std::string("OpenEXR format")}}, false);
                refresh(); 
            });
...
private:
    ...
    std::unique_ptr<PhaseFunction> m_phaseFunction;
...
int main(int argc, char **argv) {
    ...
    // CLI mode
    WarpType warpType;
    float paramValue = 0, param2Value = 0;
    std::string exrFilename;
    std::unique_ptr<BSDF> bsdf;
    std::unique_ptr<PhaseFunction> phaseFunction;
    auto bRec = BSDFQueryRecord(nori::Vector3f());
    bRec.sampler = static_cast<Sampler*>(
            NoriObjectFactory::createInstance("independent", PropertyList()));
    /* If the first argument is hierarchical_sample, parse the rest as strings, not as floats */
    for (int i = 0; i < WarpTypeCount; ++i) {
        if (strcmp(kWarpTypeNames[i].c_str(), argv[1]) == 0)
            warpType = WarpType(i);
    }
    if (warpType >= WarpTypeCount)
        throw std::runtime_error("Invalid warp type!");
    if (warpType == HierarchicalSampleWarping) {
        if (argc == 2) {
            std::cout << tfm::format("Missing EXR filename for the specified warping type") << std::endl;
            return 1;
        }
        exrFilename = argv[2];
    } else {
        ...
        if (warpType == MicrofacetBRDF) {
            ...
            bsdf.reset(ptr);
                ...
                float bsdfAngle = M_PI * 0.f;
                BSDF *ptr;
                std::tie(ptr, bRec) = WarpTest::create_microfacet_dielectric(
                    paramValue, bsdfAngle);
                bsdf.reset(ptr);
        } else if (warpType == HenyeyGreenstein) {
            float angle = M_PI * 0.f;
            PhaseFunction *ptr;
            std::tie(ptr, bRec) = WarpTest::create_henyey_greenstein(
                paramValue, angle);
            phaseFunction.reset(ptr);
        }
    }
    ...
    WarpTest tester(warpType, paramValue, exrFilename, bsdf.get(), bRec, phaseFunction.get());
    auto res = tester.run();
    if (res.first)
        return 0;
