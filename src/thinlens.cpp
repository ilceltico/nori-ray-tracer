...
/**
 * \brief Thin lens camera model for depth of field
*/
...
public:
    ThinlensCamera(const PropertyList &propList) {
        ...
        m_focus_distance = propList.getFloat("focus_distance");
        if (m_focus_distance <= 0) {
            throw NoriException("Thin Lens Camera: invalid focus distance. It must be a positive number.");
        }
        m_aperture_radius = propList.getFloat("aperture_radius");
        if (m_aperture_radius <= 0) {
            throw NoriException("Thin Lens Camera: invalid aperture radius. It must be a positive number.");
        }
    ...
    Color3f sampleRay(Ray3f &ray,
        ...
        /* Compute a pinhole ray */
        Vector3f dPinhole = nearP.normalized();

        Ray3f pinholeRay;
        pinholeRay.o = nearP;
        pinholeRay.d = dPinhole;
        pinholeRay.update();

        /* Compute the point on the focus plane */
        float ft = m_focus_distance / dPinhole.z();
        Point3f pFocus = pinholeRay(ft);

        // Sample a point on the lens
        Point2f lensPoint = m_aperture_radius * Warp::squareToUniformDisk(apertureSample);
        Point3f lensPoint3 = Point3f(lensPoint.x(), lensPoint.y(), 0);

        Vector3f d = Vector3f((pFocus - lensPoint3).normalized());

        float invZ = 1.0f / d.z();

        ray.o = m_cameraToWorld * lensPoint3;
