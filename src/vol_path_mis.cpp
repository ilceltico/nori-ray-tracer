...
public:
    ...
    Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const {
        ...
        PhaseFunction *prevPhase = nextMedium == nullptr ? nullptr : nextMedium->getPhaseFunction();
        BSDF const *prevBSDF = nextHitSurface ? nextIts.mesh->getBSDF() : nullptr;
        PhaseFunction *nextPrevPhase = nullptr;
        BSDF const *nextPrevBSDF = nullptr;
        Intersection prevIts = nextIts;
        Intersection nextPrevIts;
        Vector3f prevRayD = Vector3f(_ray->d);
        Vector3f nextPrevRayD;
        bool prevSampledMedium = nextSampledMedium;
        bool nextPrevSampledMedium;
        float prevContinueProbability = 1.0f;
        float nextPrevContinueProbability;
        Point3f prevNewPoint = nextNewPoint;
        Point3f nextPrevNewPoint;
        ...
        while (continueBouncing) {
                    ...
                    while (true) {
                        ...
                        if (shadowHitSurface) {
                            ...
                            if (sampledMedium) {
                                ...
                                if ((sampleNormal.normalized().dot((newPoint - samplePosition).normalized())) == 0.0f)
                                    p_light = 0.0f;
            ...
            if (!nextHitSurface && !nextSampledMedium) {
                if (nextIsSpecularBounce && scene->getEnvironmentMap()) {
                    Color3f emitted = scene->getEnvironmentMap()->eval(nextRay.d);
            ...
            if (nextSampledMedium) {
                ...
                nextPrevPhase = nextMedium->getPhaseFunction();
                nextPrevRayD = Vector3f(nextRay.d);
                nextPrevIts = nextIts;
                nextPrevSampledMedium = true;
                nextPrevContinueProbability = bounces < MINIMUM_BOUNCES ? 1.0f : MIN(throughput.maxCoeff() * etaProd * etaProd * eta * eta, 0.99f);
                nextPrevNewPoint = nextNewPoint;
            } else {
                nextNewPoint = nextIts.p;

                if (nextIts.mesh->getBSDF()->isNull()) {
                    nextPrevBSDF = prevBSDF;
                    nextPrevPhase = prevPhase;
                    nextPrevRayD = Vector3f(prevRayD);
                    nextPrevIts = prevIts;
                    nextPrevSampledMedium = prevSampledMedium;
                    nextPrevContinueProbability = prevContinueProbability;
                    nextPrevNewPoint = prevNewPoint;
                } else {
                    nextPrevBSDF = nextIts.mesh->getBSDF();
                    nextPrevRayD = Vector3f(nextRay.d);
                    nextPrevIts = nextIts;
                    nextPrevSampledMedium = false;
                    nextPrevContinueProbability = bounces < MINIMUM_BOUNCES ? 1.0f : MIN(throughput.maxCoeff() * etaProd * etaProd * eta * eta, 0.99f);
                    nextPrevNewPoint = nextNewPoint;
                }
            ...
            if (nextHitSurface) {
                if (!nextSampledMedium) {
                    if (nextIts.mesh->isEmitter()) {
                        ...
                        Vector3f distance = nextIts.p - prevNewPoint;

                        float p_light = nextIts.mesh->getEmitter()->pdf(newPoint, nextIts.toLocal(-nextRay.d)) * distance.squaredNorm() / Frame::cosTheta(nextIts.toLocal((newPoint - nextIts.p).normalized())) / scene->getEmitters().size();
                        ...
                        if (sampledMedium) {
                            ...
                            float p_brdf;
                            if (its.mesh->getBSDF()->isNull()) {
                                if (prevSampledMedium) {
                                    p_brdf = prevPhase->pdf(-prevRayD.normalized(), newDirection);
                                } else {
                                    BSDFQueryRecord query = BSDFQueryRecord(prevIts.toLocal(-prevRayD).normalized(), prevIts.toLocal(newDirection), ESolidAngle, sampler, &prevIts);
                                    p_brdf = prevBSDF->pdf(query);
                                }
                            } else {
                                BSDFQueryRecord query = BSDFQueryRecord(its.toLocal(-_ray->d).normalized(), its.toLocal(newDirection), ESolidAngle, sampler, &its);
                                p_brdf = its.mesh->getBSDF()->pdf(query);
                            }

                            // BSDFQueryRecord query = BSDFQueryRecord(its.toLocal(-_ray->d).normalized(), its.toLocal(newDirection), ESolidAngle, sampler, &its);
                            // float p_brdf = p_brdf = its.mesh->getBSDF()->pdf(query);
                ...
                if (!nextSampledMedium) {
                    if (scene->getEnvironmentMap()) {
                        ...
                        if (sampledMedium) {
                            float p_phase = medium->getPhaseFunction()->pdf(-_ray->d.normalized(), newDirection);
                            float mis_weight = p_phase / (p_light + p_phase);
                            // float mis_weight = 0.0f;
                            if (p_light >= 0 && p_phase > 0)
                                emitted *= mis_weight;
            ...
            prevBSDF = nextPrevBSDF;
            prevPhase = nextPrevPhase;
            prevIts = nextPrevIts;
            prevRayD = Vector3f(nextPrevRayD);
            prevSampledMedium = nextPrevSampledMedium;
            prevContinueProbability = nextPrevContinueProbability;
            prevNewPoint = nextPrevNewPoint;
